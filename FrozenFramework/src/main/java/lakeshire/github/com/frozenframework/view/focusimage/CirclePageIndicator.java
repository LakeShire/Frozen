...
public class CirclePageIndicator extends View implements PageIndicator {
    ...
    public CirclePageIndicator(Context context, AttributeSet attrs, int defStyle) {
        ...
        final float defaultExtraSpacing = res.getDimension(R.dimen
                .default_circle_indicator_extra_spacing);
        ...
        mExtraSpacing = a.getDimension(R.styleable.CirclePageIndicator_extraSpacing,
                defaultExtraSpacing);
        mIsCircle = a.getBoolean(R.styleable.CirclePageIndicator_isCircle, true);

        if (!mIsCircle) {
            rectWidth = ScreenUtil.dp2px(getContext(), 5);
            rectHeight = ScreenUtil.dp2px(getContext(), 3);
            mRadius = ScreenUtil.dp2px(getContext(), 4);
        }
...
// * threeRadius) / 2.0f);
            ...
            if (count > 1) {
                width += (count - 1) * threeRadius; //each additional circle needs 2r, plus 1r of
            }
            // padding.
            longOffset = ((longSize - longPaddingBefore - longPaddingAfter) / 2.0f) - (width / 2.0f);
            //we need to add one radius if we have more then one circle, because the draw takes
            // hte center point.
            //for one circle, we don't, because the midpoint (which is what longOffset is right
            // now) is already
        ...
        if (realcount > 0 && mLastPage == realcount - 1 && mCurrentPage == realcount - 1) {
            ...
            if (mIsCircle) {
                ...
                canvas.drawRect(x, shortOffset, x + rectWidth, shortOffset + rectHeight,
                        mPaintFill);
            }
            return;
        }
        ...
        if (realcount > 0 && mLastPage == 0 && mCurrentPage == realcount - 1) {
            if (mIsCircle) {
                ...
                canvas.drawRect(longOffset, shortOffset, longOffset + rectWidth, shortOffset +
                        rectHeight, mPaintFill);
            }
            return;
        }
